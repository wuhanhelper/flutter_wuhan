...
//    //如果顶层路由是指纹验证页，则不显示弹窗。因为其它地方有调 popUntilWithRouter(Router.gestureVerify)，假如 gestureVerify有弹窗的话，会一并被清除了.
//    if (NavigatorManager().isTopRouter(Router.gestureVerify)) {
//      return;
//    }
    //如果待显示数组为空
...
//    }
    ...
    if (NavigatorManager().isTopDialog() && _hasShowDialogList.isNotEmpty) {
      ...
      if ((bean = _getHighClearDialogBean(beans: _dialogList)) != null &&
        ...
        NavigatorManager().pop(context, popRecycle);
    ...
    //如果是强升的话，则要隐藏单例的 webview,不然弹窗不能显示 ，且 webview 返回被盖住，不能点击
...
//    flutterWebViewPlugin.hide();
  ...
  void _handlePopResult(BuildContext context, dynamic obj) {
    ...
    //如果外部手动 pop或者当前栈顶是 H5页面,则不显示下一个，否则接着显示下一个
    //处理如果弹窗打开一个 H5页面,则在h5页面在栈顶，则不显示下一个弹窗。
    if (obj != popRecycleAndNotNext &&
        obj != popNotNext/* &&
        !NavigatorManager().isTopRouter(Router.webView)*/) {
      show(context);
    }
  }
  ...
  void pop(BuildContext context, {String result}) {
    if (_hasShowDialogList.isNotEmpty && NavigatorManager().isTopDialog()) {
      NavigatorManager().pop(context, result);
    }
  }
  ...
  bool _canShow(DialogBean dialogBean) {
    ...
    if (StringUtil.isEmpty(pageRouter)) {
      return true;
    }
    //指定的页面在顶层
    if (NavigatorManager().isTopRouter(pageRouter)) {
      //如果配置有内页，且包含当前显示的内页
      if (dialogBean.innerPageRouters != null) {
        LcfarmWidgetState widgetState =
            NavigatorManager().getWidget(pageRouter);
        String innerClassName = widgetState.getInnerClassName();
        if (widgetState != null &&
            StringUtil.isNotEmpty(innerClassName) &&
            dialogBean.innerPageRouters.contains(innerClassName)) {
          return true;
        }
      } else {
        //如果没有配置内页，则直接返回 true
    ...
    return false;
//    return StringUtil.isEmpty(routerName) ||
//        NavigatorManager().isTopRouter(routerName);
  }
